...
const langs = {
  en: {
    ...
    demoLead: "Explore a visual demo of the Ultimate Clipboard Pro interface mounted on this page with a secure demo data bridge. It is visual only: capture, copy, tools, settings, classification, deletion and paid workflows are disabled, but you can open the launcher, browse captures, search, reorder categories and switch versions.",
  ...
  it: {
    ...
    demoTitle: "Demo interattiva di Ultimate Clipboard Pro", demoDesc: "Prova una demo visiva di Ultimate Clipboard Pro con launcher flottante, pannello laterale, gestore, categorie, versioni e catture di esempio.", demoKicker: "Demo prodotto interattiva", demoLead: "Esplora una demo visiva dell’interfaccia Ultimate Clipboard Pro montata su questa pagina con un bridge sicuro di dati demo. È solo visiva: cattura, copia, strumenti, impostazioni, classificazione, eliminazione e flussi a pagamento sono disattivati, ma puoi aprire il launcher, sfogliare catture, cercare, riordinare categorie e cambiare versione.",
  ...
  de: {
    ...
    demoTitle: "Interaktive Demo von Ultimate Clipboard Pro", demoDesc: "Testen Sie eine visuelle Demo von Ultimate Clipboard Pro mit Floating Launcher, Seitenfenster, Manager, Kategorien, Versionen und Beispiel-Captures.", demoKicker: "Interaktive Produktdemo", demoLead: "Erkunden Sie eine visuelle Demo der Ultimate Clipboard Pro Oberfläche, die auf dieser Seite mit einer sicheren Demo-Datenbrücke läuft. Sie ist rein visuell: Erfassen, Kopieren, Tools, Einstellungen, Klassifizierung, Löschen und Bezahlabläufe sind deaktiviert, aber Sie können den Launcher öffnen, Captures durchsuchen, suchen, Kategorien neu ordnen und Versionen wechseln.",
...
function staticProductPage(lang, page) {
  ...
  const main = page === "demo"
    ? `<section class="ucp-demo-intro"><article><h2>${esc(l.demoTitle)}</h2><p class="ucp-demo-instruction">${esc(l.demoAllowed || "Click the floating buttons in the bottom-right corner to try the extension interface.")}</p><p>${esc(l.demoLead)}</p><ul><li>${esc(l.demoBrowse || "Open the floating launcher and browse a realistic Pro workspace.")}</li><li>${esc(l.demoSafe || "Demo mode cannot capture, copy, open settings or run tools.")}</li></ul></article></section><div class="ucp-demo-root" data-ucp-demo-lang="${lang}"></div>`
    : page === "faq"
